...
This is already included at `.github/workflows/build.yml` in this repo:
...
`setup-java` with JDK 17 is required — Android Gradle Plugin 8.3 (used here) won't build
on the runner's default JDK. Trigger the workflow manually from the **Actions** tab (or
just push a commit) and download the APK artifact from the run — same pattern as your
finance tracker app.
