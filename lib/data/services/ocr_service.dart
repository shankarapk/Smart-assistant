...
class OcrService {
  ...
  // plain letter — most often R or Z, occasionally the digit 3 — glued
  // directly to the number with no space (e.g. "R64900", "Z899"). This
  // fallback only fires when the primary pattern above finds nothing, and
  // matches are flagged lowConfidence so the Review screen can call them
  // out for extra scrutiny.
  static final _priceMisreadRegex =
      RegExp(r'(?<![A-Za-z0-9])[RZ3](\d{2,6})(?:\.(\d{1,2}))?(?![A-Za-z0-9])');
