...
class OcrService {
  ...
  // Matches a badge word glued to the front or back of an otherwise-real
  // name, e.g. "OFF Drumstick" (ML Kit merged a discount badge and the
  // actual product name into one text block).
  static final _badgeWordEdge = RegExp(
    r'^(off|add|new|sale|save|mrp|flat|extra|combo|free|offer|offers|buy|best|deal|deals)\s+'
    r'|\s+(off|add|new|sale|save|mrp|flat|extra|combo|free|offer|offers|buy|best|deal|deals)$',
    caseSensitive: false,
  );

  String _stripBadgeWords(String input) {
    var result = input.trim();
    var match = _badgeWordEdge.firstMatch(result);
    while (match != null) {
      result = result.replaceFirst(_badgeWordEdge, '').trim();
      match = _badgeWordEdge.firstMatch(result);
    }
    return result;
  }
